...
/**
 * Jira issue için atanabilir kullanıcıları getir
 */
export async function GET(
  ...
  try {
    ...
    const projectKey = issueKey.split("-")[0]; // Issue key'den project key'i çıkar (örn: PROJ-123 -> PROJ)
    
    // Jira API endpoint'i: /rest/api/3/user/assignable/search
    // issueKey veya projectKey ile kullanılabilir
    let assignableUsersEndpoint = `${apiUrl}/rest/api/3/user/assignable/search?maxResults=50`;
    
    // Issue key ile arama yap
    if (issueKey) {
      assignableUsersEndpoint += `&issueKey=${issueKey}`;
    } else if (projectKey) {
      assignableUsersEndpoint += `&project=${projectKey}`;
    }
    
    // Query parametresi varsa ekle
    if (query.trim()) {
      assignableUsersEndpoint += `&query=${encodeURIComponent(query.trim())}`;
    }
    
    const response = await fetch(assignableUsersEndpoint, {
      headers: {
        Authorization: `Bearer ${jiraToken}`,
        Accept: "application/json",
      },
